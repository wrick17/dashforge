...
export const Renderer = ({ layout }) => {
	...
	const handleHorizontalResize = (rowIndex, paneIndex, delta) => {
		setPaneSizes(prevSizes => {
			...
			if (paneIndex < rowSizes.length - 1) {
				...
				const newCurrentSize = Math.max(minSize, Math.min(maxSize, rowSizes[paneIndex] + deltaPercent));
				const newNextSize = Math.max(minSize, Math.min(maxSize, rowSizes[paneIndex + 1] - deltaPercent));
	...
	const handleVerticalResize = (rowIndex, delta) => {
		setRowHeights(prevHeights => {
			...
			if (rowIndex < newHeights.length - 1) {
				...
				const newCurrentSize = Math.max(minSize, Math.min(maxSize, newHeights[rowIndex] + deltaPercent));
				const newNextSize = Math.max(minSize, Math.min(maxSize, newHeights[rowIndex + 1] - deltaPercent));
	...
	return (
		<div className="renderer" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
			{layout.map((row, rowIndex) => {
				const currentRowSizes = paneSizes[rowIndex] || Array(row.length).fill(totalWidth / row.length);
